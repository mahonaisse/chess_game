...
This class diagram represents what classes and functions that we plan to implement. We first have a Chess Game as the big main class, where all the other classes are derived from. The Chess Game class has two players and board. The Ghess Game class also has the functions play() which will allow you to chose newGame() or loadGame(), newGame() which creates a new game, loadGame() which will load an existing game, and quit() which will quit the program. Then in the Player class we have a string name where a player can enter their name and vector of pieces. Each player has a function called playTurn() where the player will move one of their pieces. Player 1, Player 2, and CPU classes are all derived from Player class. CPU has a function called randomMove() which will make a legal random move. The Board class is a 2d array of Pieces. After each move the Board class will call a function call updateBoard() to update the position of the board. ConstructBoard() will construct a new board compliant with tournament rules. check() will see if the current position has a check, and checkmate() will see if the current position has a checkmate. The Piece class is an abstract class. In the Piece Class it keeps track if its current location on the board through it rowPos and columnPos. It will also have a type of piece it is and color black or white. It also has the three functions moveIsLegal() where it checks if the move is legal, moveTo() where a piece would move to a place only if moveIsLegal() returns true, and die() when a piece gets captured by an enemy piece. Pawn, Bishop, King, Rook, Queen, and Kinght are all derived from the Piece class and have those functions. King has a seperate function called checkmate() where it sees if is in checkmate().
